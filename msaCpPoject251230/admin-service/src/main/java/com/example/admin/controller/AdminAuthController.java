...
public class AdminAuthController {
    ...
    public ResponseEntity<?> createAdmin(@RequestBody CreateAdminRequest req) {
        try {
            ...
            // ✅ SHA-512(PBKDF2WithHmacSHA512)로 해시 저장
            u.setPw(passwordEncoder.encode(req.password()));
    ...
    public ResponseEntity<?> update(@PathVariable String loginId, @RequestBody UpdateAdminRequest req) {
        try {
            var user = adminUserRepository.findByLoginIdAndDelYn(loginId, "N")
                    .orElseThrow(() -> new IllegalArgumentException("해당 관리자를 찾을 수 없습니다."));
            ...
            if (req.password() != null && !req.password().isBlank()) {
                user.setPw(passwordEncoder.encode(req.password()));
            }
