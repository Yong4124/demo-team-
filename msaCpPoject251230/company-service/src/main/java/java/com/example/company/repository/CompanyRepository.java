package java.com.example.company.repository;
...
public interface CompanyRepository extends JpaRepository<Company, Long> {
    ...
    // 검색 (회사명 또는 담당자명)
    @Query("SELECT c FROM Company c WHERE c.delYn = :delYn AND " +
           "(c.companyNmKo LIKE %:keyword% OR c.companyNmEn LIKE %:keyword% OR c.managerNm LIKE %:keyword%)")
    List<Company> searchByKeyword(@Param("keyword") String keyword, @Param("delYn") DelYn delYn);

    // Admin 통계용: 기간별 가입 수
    @Query("SELECT FUNCTION('DATE_FORMAT', c.insertDate, '%Y-%m') as month, COUNT(c) as cnt " +
           "FROM Company c WHERE c.insertDate BETWEEN :from AND :to AND c.delYn = 'N' " +
           "GROUP BY FUNCTION('DATE_FORMAT', c.insertDate, '%Y-%m') ORDER BY month")
    List<Object[]> countByInsertDateBetween(@Param("from") LocalDate from, @Param("to") LocalDate to);
